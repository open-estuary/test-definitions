...
```bash
  1.board_reboot 
  2.board_connect 
  3.fio -name=write -numjobs=32 -filename=/dev/sda -rw=write -iodepth=128 -direct=1 -sync=0 -norandommap -group_reporting -runtime=30 -time_base -bs=4kash
...
```bash
  ...
  2.lsi raid3108 card sequential write bandwidth is close to theoretical value.
```
