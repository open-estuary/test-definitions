...
############################################################################
#用例名称：$rt-migrate-test_function.sh
#用例功能：
#	验证rt-migrate-test可否实现实时任务迁移测试
#
#
#作者：swx703520
#完成时间：2019/5/11
...
function test1()
{
#####rt-migrate-test的基本功能实现（验证rt-migrate-test可否实现实时任务迁移测试）#####
	./rt-migrate-test -l 100 
	if [ $? -eq 0 ];then
	    ###@把测试的结果写入到结果文件中###
        write_result "${RESULT_FILE}" "migrate_command" "pass"
    else
        write_result "${RESULT_FILE}" "migrate_command" "fail"
    fi
}
################################基本功能测试################################
###基本功能测试###
