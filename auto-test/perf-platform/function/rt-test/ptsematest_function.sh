...
############################################################################
#用例名称：$ptsematest_function.sh
#用例功能：
#验证ptsematest可否实现进程间mutex通信测试
#作者：swx703520
#完成时间：2019/5/11
#版本号  ： V0.1
##############################################################################
...
{
#####验证ptsematest可否实现进程间mutex通信测试#####
#####当前处理器-a,设置-t2线程，优先级-p99，线程间隔时间-i100，间隔距离-d25，循环-l1000000#####
    timeout 15      ./ptsematest -a -t2 -p99 -i100 -d25 -l10000 
	if [ $? -eq 0 ];then
	    ###@把测试的结果写入到结果文件中###
        write_result "${RESULT_FILE}" "pts_command" "pass"
    else
        write_result "${RESULT_FILE}" "pts_command" "fail"
    fi
}
