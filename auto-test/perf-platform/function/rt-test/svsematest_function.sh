...
############################################################################
#用例名称：svsematest_function
#用例功能：
#        1.启动多个进程，每个进程迭代收发1000次SYSV信号消息
#作者    ：何军健 hwx573498
#完成时间：2019/05/14
#版本号  ： V0.1
##############################################################################
...
set -x
path=`pwd`
LOOPS="1000" #svsematest -l 设置进程迭代收发SYSV信号的次数
PRIORITY="99"  #svsematest -p 设置进程的优先级
INTERVAL="100" #svsematest -i 设置线程的基本间隔
DISTANCE="25" #svsematest -d 设置线程的间隔距离
...
################################测试函数实现区域################################
#运行svsematest进行测试
function svsematest_run(){
    ./svsematest -a -t -p "${PRIORITY}" -i "${INTERVAL}" -d "${DISTANCE}" -l "${LOOPS}" | tee ${TMPFILE}
    #提取日志文件
    #查看最后一个进程程的最小延迟
    ...
    if [ ${min_latency} -ne 0 -a ${avg_latency} -ne 0 -a ${max_latency} -ne 0 ];then
        ###@把测试的结果写入到结果文件中###
        write_result "${RESULT_FILE}" "svsematest" "pass"
    else
        write_result "${RESULT_FILE}" "svsematest" "fail"
    fi
}
