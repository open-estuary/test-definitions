...
sched_pin(int cpu)
{
...
#if 1
	cpu_set_t mask, cpumask;
	size_t sz = sizeof (cpu_set_t);
	int i, j;
	unsigned char *p = &mask, *cp = &cpumask;

	retval = sched_getaffinity(0, sz, &cpumask);
	if (retval < 0) perror("sched_getaffinity:");
	if (retval < 0) return retval;

	//CPU_ZERO(&mask);
	bzero(&mask, sz);
	for (i = 0, j = 0; i < sz * 8; i++) {
		int n = i / 8;
		int r = i % 8;
		if (cp[n] & (1 << r)) {
			if (j == cpu) {
	...
	retval = sched_setaffinity(0, sz, &mask);
#else
	/* Linux interface */
